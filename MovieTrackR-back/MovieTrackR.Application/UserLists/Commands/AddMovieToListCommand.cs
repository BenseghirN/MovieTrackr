...
public sealed class AddMovieToListHandler(IMovieTrackRDbContext dbContext, IMediator mediator)
    : IRequestHandler<AddMovieToListCommand>
{
    public async Task Handle(AddMovieToListCommand command, CancellationToken cancellationToken)
    {
        ...
        Guid movieId = await mediator.Send(new EnsureLocalMovieCommand(command.MovieId, command.TmdbId), cancellationToken);
